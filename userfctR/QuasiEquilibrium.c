...
void QuasiEquilibrium(MbsData *mbs_data, double V, double front_radius, double rear_radius, int Toprint, double steer)
{
	...
	if (mbs_data->tourne == 1)
	{
		...
		mbs_equil->options->quch[2] = T2_body_id;
		...
		mbs_equil->options->xch_ptr[2] = &(mbs_data->Qq[R3_steering_fork_id]);
	...
	else
	{
		...
		mbs_data->Qq[R2_wheel_rr_id] = 0.001 * 15 * 9.81;
		...
		mbs_equil->options->equitol = 1e-5;
